...
Let $n = \dim(q)$.
By \Cref{M-C:diagonalisation} we may assume without loss of generality $q = \langle a_1, \ldots, a_n \rangle_\qq$ for some $a_1, \ldots, a_n \in \qq$, and in fact we have $a_1, \ldots, a_n \in \qq^\times$ since we assume that $q$ is anisotropic.
...
In view of the classification of square classes (\Cref{M-C:qqp-squares-odd} and \Cref{M-C:qq2-squares}) we may assume there exists $z_p \in \zz \cap D_{\qq_p}(\langle a_1, a_2 \rangle_{\qq_p}) \cap D_{\qq_p}(-\langle a_3, a_4 \rangle_{\qq_p})$ with, for $p \in \mbb P$, $v_p(z_p) \leq 1$.
