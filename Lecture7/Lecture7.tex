...
\end{align*}
so we can apply Hensel's Lemma (\Cref{M-P:Hensel}). We get that there exists $\beta\in\mathbb{Z}_{p}$ such that $f(\beta )=0$. By the quadratic formula, 
\begin{align*}
...
In the remaining case $k_1 = k_2 = 1$, it suffices to use the computation rules from \Cref{C:K-theory-axioms}: we have
...
But this table can be established entirely via the computation rules of \Cref{M-P:K-theory-axioms} and \Cref{M-C:K-theory-axioms}.
\end{proof}

\subsection{Exercises}
\begin{enumerate}
\item Complete the proof of \Cref{C:qq2-squares} and \Cref{P:2-fold-Pfister-qq2}.
